...
void UNotifyState_RandomLighting::NotifyBegin(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, float TotalDuration, const FAnimNotifyEventReference& EventReference)
{
	...
	if (world)
	{
		...
		/*	niagaraComp[i] = UNiagaraFunctionLibrary::SpawnSystemAtLocation(world, magicCircle, spawnPos[i], FRotator::ZeroRotator);

			if (niagaraComp[i])
			{
				niagaraComp[i]->SetWorldScale3D(FVector(0.5f, 0.5f, 0.5f));
			}*/
